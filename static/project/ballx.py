...
class main(Tkinter.Tk):
    ...
    def creating_line(self):
        ...
        self.lin = self.board.create_rectangle(PLAYGROUND_WIDTH - x1, 0, PLAYGROUND_WIDTH - x1 - 50, 10, tag="l2",
                                               outline="yellow")
        self.line = self.board.create_rectangle(PLAYGROUND_WIDTH - x1 - 50, 0, 0, 10, fill="powder blue", tag="l1",
                                                outline="yellow")
        self.line1 = self.board.create_rectangle(PLAYGROUND_WIDTH, 0, PLAYGROUND_WIDTH - x1, 10, fill="powder blue",
                                                 tag="l1", outline="yellow")

        return

    def creating_ball(self):
        self.ball = self.board.create_oval(1, PLAYGROUND_HEIGHT / 2, 11, PLAYGROUND_HEIGHT / 2 - 10, fill="black",
                                           tag="b")
        return
    ...
    def game_loss(self):
        self.board.create_text(PLAYGROUND_WIDTH / 2, PLAYGROUND_HEIGHT / 2, text="Game Over", font='arial 60 bold',
                               fill='red')
        self.board.create_text(PLAYGROUND_WIDTH / 2, PLAYGROUND_HEIGHT / 2 + 70,
                               text="SCORE: {}".format(int(self.score / 11)), font='arial 20 bold',
                               fill='powder blue')
        global gamevalid
        gamevalid = 0
        return
    ...
    def update_line(self):
        ...
        if self.ball_target is None:
            ...
            if (len(self.board.find_overlapping(x5, y5, x6, y6)) > 2) or (
                    len(self.board.find_overlapping(x7, y7, x8, y8)) > 2):
